...
var PERF_DISPLAY_MAP = { '超出': '超出', '略超': '略超', '达到': '达到', '略低': '略低', '低于': '未达' };

/** 绩效标签 CSS 类映射 */
var PERF_CLASS_MAP = { '达到': 'perf-met', '略低': 'perf-slight-below', '低于': 'perf-below', '略超': 'perf-slight-exceed', '超出': 'perf-exceed' };

/**
 * 获取人员当前应展示的绩效标签（考虑待填报+绩效任务场景）
 * @returns [{text, displayText, cssClass}, ...] 最多 2 个
 */
function getEffectivePerformanceTags(emp) {
    ...
    return [
        { text: tags[0] && tags[0].text || '达到', displayText: PERF_DISPLAY_MAP[tags[0] && tags[0].text] || (tags[0] && tags[0].text) || '达到', cssClass: PERF_CLASS_MAP[tags[0] && tags[0].text] || 'perf-met' },
        { text: tags[1] && tags[1].text || '达到', displayText: PERF_DISPLAY_MAP[tags[1] && tags[1].text] || (tags[1] && tags[1].text) || '达到', cssClass: PERF_CLASS_MAP[tags[1] && tags[1].text] || 'perf-met' }
    ];
}
 ...
 * @param {Object} opts - { compact: 是否紧凑(列表用), showDelta: 是否显示调薪增幅, showOriginalLine: 是否显示原薪资行 }
 */
function buildPersonCardContent(emp, opts) {
    ...
    var perfHtml = '<span class="scatter-card-perf ' + (t1.cssClass || '') + '" title="' + (t1.text || '') + '">' + (t1.displayText || '') + '</span><span class="scatter-card-perf ' + (t2.cssClass || '') + '" title="' + (t2.text || '') + '">' + (t2.displayText || '') + '</span>';
    ...
    if (showOriginalLine && !compact && emp.adjustment !== 0 && typeof cardGhosts !== 'undefined') {
        ...
        var origTier = (cardGhosts[emp.id] && cardGhosts[emp.id].originalLevel != null) ? String(cardGhosts[emp.id].originalLevel) : currTier;
